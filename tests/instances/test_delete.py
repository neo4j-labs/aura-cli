...
def test_delete_instance(api_request):
    runner = CliRunner()

    api_request.return_value = mock_response()

    result = runner.invoke(delete_instance, ["--instance-id", "123", "--yes"])
...
def test_delete_instance_with_name(api_request):
    runner = CliRunner()
    ...
    result = runner.invoke(delete_instance, ["--name", "Instance01", "--yes"])

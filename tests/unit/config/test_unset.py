...
def test_unset_config_option(mock_config):
    ...
    result = runner.invoke(unset_option, ["default_tenant"], obj=mock_config)

    assert result.exit_code == 0
    assert result.output == "Config option default_tenant unset\n"

    mock_config.unset_option.assert_called_once_with("default_tenant")

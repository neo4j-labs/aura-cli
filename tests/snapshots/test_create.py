...
def test_create_snapshot(api_request):
    runner = CliRunner()

    api_request.return_value = mock_response()

    result = runner.invoke(create_snapshot, ["--instance-id", "123"])
...
def test_create_snapshot_with_name(api_request):
    runner = CliRunner()
    ...
    result = runner.invoke(create_snapshot, ["--instance-name", "Instance01"])
